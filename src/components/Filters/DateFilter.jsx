    ...
    <div className="p-2 bg-gray-400 rounded flex justify-start items-center space-x-3">
      <label className="text-gray-800">
        ...
        <input
          ...
          type="date"
          value={dateFrom}
          min={formatedMinDateLimit}
          max={dateTo}
          className="h-8 py-1 text-sm text-gray-800 rounded"
      ...
      <label className="text-gray-800">
        ...
        <input
          ...
          value={dateTo}
          min={dateFrom}
          max={formatedMaxDateLimit}
          className="h-8 py-1 text-sm text-gray-800 rounded"
          onChange={(event) => setDateTo(event.target.value)}
        />
      </label>
      <Link
        to={`${location.pathname}?dateFrom=${
          dateFrom ? dateFrom : dateTo ? formatedMinDateLimit : ""
        }&dateTo=${dateTo ? dateTo : dateFrom ? formatedMaxDateLimit : ""}`}
        className="py-1 px-4 rounded space-x-4 flex justify-start items-center bg-blue-400 hover:bg-blue-500 active:bg-blue-800"
        onClick={() => {
          props.handleClick();
        }}
      >
